...
- hindsight-automatic-memory: [Automatic memory](../../README.md#hindsight-automatic-memory) - `autoReflect: true` restores first-prompt synthesis, capped at 20 seconds by the installed hook. Reflection/consolidation use Terra; retain defaults to Luna. Explicit-tool/MCP/server deadlines: 660/720/1000 seconds. Knowledge pages, capture, ingestion and `shared` remain enabled; start a new session or reconnect MCP to refresh cached configuration.
